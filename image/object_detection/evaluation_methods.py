1.物体検出精度
　画像中の検出したい物体を、作成したモデルがどの程度正しく検出できるかを評価する指標
  機械学習で基本的な評価指標であるPrecision、Recall等を組み合わせ、複数の画像に対してモデルがどの程度機能しているかを評価する
  # 代表的な評価指標：mAP、IoU

2.処理速度
  比較的データの大きくなりがな画像を、機械学習、特にDeep Learningで扱うとなれば、処理時間が膨大となる。
  危険回避のため高い処理速度が求められる自動運転等では、特にこの処理速度の高速化は重要
  物体検出モデルを評価する指標として、fps(Frames per second)がある
  名前の通り、一秒間あたりに何枚の画像を処理できるか、という指標です。
  高速物体検出手法として良く知られているYOLO(https://pjreddie.com/darknet/yolo/) は、cocoデータセット(coco test-dev)に対して30[fps]の処理速度を誇ります。今回はfpsを測定していきます。
...
# IoU
Intersection(領域の共通部分) over Union(領域の和集合)
over は、割り算
R1...Ground-truthの領域 , R2...切り抜いた矩形 , Rol...R1とR2の重なっている領域
IoU = |Rol| / |R1 + R2 - Rol|

一つの矩形を設定して(GT: Ground-truth, 正解と呼ぶ)、ランダムに切り抜いた矩形がGTとある程度重なっているかを評価する

IoU は厳しい評価指標
同じ正方形を斜めに 19 ズラしただけでも、IoU は 6498≒0.65 になる
